// See: http://p5js.org/examples/examples/Dom_Video_Capture.php
// See: https://github.com/processing/p5.js/wiki/Local-server
// To run this example locally in a browser, you will need the p5.dom library 
...
function setup() {
  ...
  modes = ["1", "2", "3", "4", "5", "6", "P", "E"]; // modes only for instruments
  mode = "1";
...
function playRipplers(){
  for(var i = 0; i < ripplers.length; i++){
    if(ripplers[i].play(pickColor) === true){
...
function playBubblers(){
  for(var i = 0; i < bubblers.length; i++){
    bubblers[i].play(pickColor);
    
    if(bubblers[i].exploded){

      makeBubbles(bubblers[i].x, bubblers[i].y);
    }
  }
}
...
function Ball(x, y, wid, hei){
  ...
  this.updateSteps = function(pickColor){
    var error = 0.215;
...
function Control(y, mode){
  ...
  this.modeEText = "Erase at you leisure."
